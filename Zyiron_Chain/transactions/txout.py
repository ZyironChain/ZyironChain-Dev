...
class TransactionOut:
    ...
    def _calculate_tx_out_id(self) -> str:
        ...
        try:
            # Combine the components into a string and encode to bytes
            data = f"{self.script_pub_key}{self.amount}{self.locked}".encode()

            # Use the Hashing class to perform double-hashing based on the current setting
            return Hashing.hash(data)  # Uses the configured double hashing logic
    ...
    def from_dict(cls, data: Dict) -> "TransactionOut":
        ...
        try:
            ...
            raise ValueError(f"[ERROR] Invalid amount format: {e}")
        if not script_pub_key:
            logging.warning("[WARNING] script_pub_key is missing. Defaulting to empty string.")
            script_pub_key = ""
