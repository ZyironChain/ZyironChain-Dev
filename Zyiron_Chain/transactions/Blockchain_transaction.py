...
# In your TransactionFactory class
...
from typing import List, Dict
from Zyiron_Chain.blockchain.constants import Constants
# (Assume Transaction, TransactionIn, TransactionOut, and TransactionType are imported appropriately)
...
class TransactionFactory:
    ...
    def create_transaction(tx_type: "TransactionType", inputs: List[Dict], outputs: List[Dict], poc=None) -> "Transaction":
        ...
        """
        if poc is None:
            poc = PoC(storage_type=Constants.DATABASES["blockchain"])  # Now PoC accepts storage_type
        ...
        prefix = (Constants.TRANSACTION_MEMPOOL_MAP[tx_type]["prefixes"][0]
                  if tx_type != TransactionType.STANDARD else "")
