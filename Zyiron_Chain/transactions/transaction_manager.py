...
class TransactionManager:
    ...
    def create_transaction(self, sender, recipient_address=None, amount=None, fee=None, transaction_type="STANDARD", **kwargs):
        ...
        Ensures:
        - Transactions retrieved from `tx_storage` are validated against block storage offsets.
        ...
        try:
            ...
            try:
                ...
                print("[TransactionManager.create_transaction ERROR] Invalid amount or fee format.")
                raise ValueError("Amount and fee must be valid decimal values.")

            print(f"[TransactionManager.create_transaction INFO] Creating transaction from {sender} to {recipient_address} for {amount} ZYC.")

            # ✅ **Ensure amount is not zero or negative**
            if amount <= 0:
                print("[TransactionManager.create_transaction ERROR] Amount must be greater than zero.")
                raise ValueError("Transaction amount must be positive.")

            # ✅ **Ensure fee meets minimum requirements**
            if fee < Decimal(Constants.MIN_TRANSACTION_FEE):
                print(f"[TransactionManager.create_transaction ERROR] Fee too low. Adjusting to minimum {Constants.MIN_TRANSACTION_FEE}.")
            ...
            if sender_balance < amount + fee:
                print(f"[TransactionManager.create_transaction ERROR] Insufficient balance. Required: {amount + fee}, Available: {sender_balance}")
            ...
            if self.tx_storage.transaction_exists(tx_hash):
                print(f"[TransactionManager.create_transaction ERROR] Duplicate transaction ID detected: {tx_hash}")
                raise ValueError("Duplicate transaction ID detected. Aborting.")

            # ✅ **Retrieve transaction storage offset for validation**
            block_offset = self.block_storage.get_block_offset(tx_hash)
            if block_offset is None:
                print(f"[TransactionManager.create_transaction ERROR] No valid block storage offset found for transaction {tx_hash}.")
                raise ValueError("Invalid transaction storage reference.")
            ...
            transaction = {
                ...
                "storage_offset": block_offset  # Ensure block storage offset is included
            }

            # ✅ **Convert raw transaction bytes (if needed)**
            serialized_tx = json.dumps(transaction, sort_keys=True).encode("utf-8")

            # ✅ **Store transaction in `tx_storage`**
            self.tx_storage.store_transaction(tx_hash, serialized_tx)

            print(f"[TransactionManager.create_transaction SUCCESS] Transaction {transaction['tx_id']} created and stored successfully.")
            return transaction
    ...
    def store_transaction_in_mempool(self, transaction: Transaction) -> bool:
        ...
        Fixes:
        - Ensure the recipient’s scriptPubKey is correctly formatted in bytes.
        - Verify UTXO locking & unlocking logic is correctly applied.
        - Implement signature validation before broadcasting.
        ...
        try:
            ...
            for output in transaction.outputs:
                if isinstance(output.script_pub_key, str):
                    ...
                    print(f"[TransactionManager.store_transaction_in_mempool] ERROR: Invalid scriptPubKey format for transaction {transaction.tx_id}.")
                    return False
            ...
            if success:
                ...
                # ✅ **Store transaction metadata offsets dynamically for efficient lookup**
                metadata_offset = self.tx_storage.store_transaction(transaction.tx_id, json.dumps(transaction.to_dict(), sort_keys=True).encode("utf-8"))
    ...
    def select_transactions_for_block(self, max_block_size_mb: int = 10):
        ...
        Transactions are selected based on highest fee-per-byte and must fit within block limits.
        """
        try:
            max_size_bytes = max_block_size_mb * 1024 * 1024
            chain_height = self._get_chain_height()

            # Allocation for smart, standard, instant payments
            ...
            for tx in sorted_txs:
                ...
                # ✅ Verify SHA3-384 hash consistency between computed `tx_id` and stored LMDB index
                ...
                for tx_in in tx.inputs:
                    ...
                    # UTXO check passed, proceed with allocation
                    ...
                    # ✅ Check overall limit before adding transaction
            ...
            # ✅ Ensure block storage offsets are updated after mempool storage
            for tx in selected_txs:
                self.tx_storage.store_transaction_in_lmdb(tx)
    ...
    def _calculate_transaction_size(self, tx: Transaction) -> int:
        ...
        try:
            # ✅ Ensure `tx_id` is in string format
            if isinstance(tx.tx_id, bytes):
                tx.tx_id = tx.tx_id.decode("utf-8")

            # ✅ Compute SHA3-384 hash for verification
            computed_tx_id = hashlib.sha3_384(tx.tx_id.encode()).hexdigest()

            # ✅ Fetch stored transaction from LMDB to verify integrity
            stored_tx_data = self.tx_storage.get_transaction(tx.tx_id)

            if not stored_tx_data or computed_tx_id != tx.tx_id:
                print(f"[TransactionManager._calculate_transaction_size] ERROR: Transaction ID mismatch or missing from LMDB for {tx.tx_id}. Skipping size calculation.")
                return -1

            # ✅ Calculate base size (Transaction ID + Timestamp + Nonce)
            base_size = len(computed_tx_id) + len(str(tx.timestamp)) + len(str(tx.nonce))

            # ✅ Calculate Input & Output sizes (Ensure JSON size calculation is accurate)
            ...
            # ✅ Adjust size calculation based on transaction metadata
            metadata_size = len(str(tx.block_height)) + len(str(tx.priority_flag))

            # ✅ Dynamically Adjust Mempool Expiration Based on Network Congestion
            mempool_expiry_adjustment = 0
            if Constants.MEMPOOL_TRANSACTION_EXPIRY > 7200:  # If expiration > 2 hours, reduce size impact
                mempool_expiry_adjustment = -50  # Reduce estimated size to optimize block inclusion
            elif Constants.MEMPOOL_TRANSACTION_EXPIRY < 3600:  # If expiration < 1 hour, increase size impact
                mempool_expiry_adjustment = 50  # Increase estimated size for longer-lived transactions
    ...
    def validate_transaction(self, tx: Transaction) -> bool:
        ...
        - Dynamically allocate block storage offsets before block inclusion.
        - Ensure transaction priority sorting considers UTXO age and fee-per-byte ratio.
        - Enforce mempool expiration for outdated transactions.
        - Ensures byte-level validation before storing.
        - Verifies SHA3-384 hash consistency between computed transaction ID and stored LMDB transaction index.
        - Cross-checks UTXO spending rules before adding to the mempool.
        - Ensures block storage offsets are updated after mempool storage.
        """
        try:
            from Zyiron_Chain.transactions.coinbase import CoinbaseTx
            ...
            for tx_in in tx.inputs:
                ...
                if not utxo:
                    ...
                    return False
                if getattr(utxo, "locked", False) or utxo["spent_status"] == 1:
                    print(f"[TransactionManager.validate_transaction] ERROR: UTXO {tx_in.tx_out_id} is locked or already spent.")
                    return False
            ...
            # ✅ **Ensure block storage offsets are updated after mempool storage**
            storage_offset = self.tx_storage.store_transaction_in_lmdb(tx)
            if storage_offset is None:
                print(f"[TransactionManager.validate_transaction] ERROR: Failed to store transaction offsets for {tx.tx_id}.")
                return False

            print(f"[TransactionManager.validate_transaction] ✅ SUCCESS: Transaction {tx.tx_id} validated and stored at offset {storage_offset}.")
            return True
    ...
    def get_pending_transactions(self, block_size_mb: int = None):
        ...
        # ✅ Step 2: Validate Transactions Against LMDB
        validated_txs = []
        for tx in all_txs:
            ...
            for tx_in in tx.inputs:
                utxo = self.utxo_manager.get_utxo(tx_in.tx_out_id)
                if not utxo or getattr(utxo, "locked", False) or utxo["spent_status"] == 1:
                    print(f"[TransactionManager.get_pending_transactions] ERROR: UTXO {tx_in.tx_out_id} is invalid, locked, or already spent.")
                    valid_utxo = False
                    break
    ...
    def sign_tx(self, transaction: Transaction):
        ...
        Fixes Applied:
        - ✅ Ensure transaction signatures are validated against LMDB transaction index storage.
        - ✅ Implement byte-level validation to confirm UTXO offsets align with stored outputs.
        - ✅ Dynamically apply congestion-based fee scaling via Constants.MIN_TRANSACTION_FEE.
        """
        with self.mempool_lock:
            try:
                ...
                for tx_in in transaction.inputs:
                    ...
                    # ✅ Ensure UTXO offsets align with stored outputs in LMDB
                    stored_utxo = self.utxo_manager.get_utxo(tx_in.tx_out_id)
                    if not stored_utxo:
                        print(f"[TransactionManager.sign_tx] ERROR: UTXO {tx_in.tx_out_id} not found in LMDB storage.")
                        return False

                    # ✅ Validate UTXO offset alignment
                    stored_offset = stored_utxo.get("data_offset", None)
                    if stored_offset is None:
                        print(f"[TransactionManager.sign_tx] ERROR: Missing data offset for UTXO {tx_in.tx_out_id}.")
                        return False

                    # ✅ Compute signature ensuring UTXO offset consistency
                    signature_data = f"{tx_in.tx_out_id}{private_key}{stored_offset}"
                    tx_in.script_sig = hashlib.sha3_384(signature_data.encode()).hexdigest()

                # ✅ Verify signature consistency against LMDB transaction index storage
                ...
                # ✅ Ensure signed transaction matches stored index
                stored_signature = stored_tx_data.get("tx_signature_hash", "")
                if stored_signature != transaction.inputs[0].script_sig:
                ...
                print(f"[TransactionManager.sign_tx] ✅ SUCCESS: Transaction {transaction.tx_id} successfully signed.")
